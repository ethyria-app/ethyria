...
for (const lang of LANGS) {
  for (const sym of SYMBOLS) {
    ...
    const faqSectionRe =
      /<!-- FAQ -->\s*<section[^>]*>\s*<h2[^>]*>.*?<\/h2>\s*([\s\S]*?)<\/section>/;
    ...
    if (
      !html.includes("faqAccordionSymbol") ||
      !html.includes("initFaqAccordionSymbol")
    ) {
    ...
    html = html.replace(
      ...
      (match) => {
        ...
        return match.replace(
          'class="mb-10 p-6 rounded-2xl"',
          'class="mb-10 text-center p-6 rounded-2xl"',
        );
      },
    );
