...
for (const filePath of files) {
  ...
  if (!html.includes('og:image:width')) {
    ...
    const ogImagePattern = /(property="og:image"\s*\n?\s*content="[^"]*"\s*\n?\s*\/>)/;
  ...
  if (!html.includes('twitter:image')) {
    ...
    const twitterDescPattern = /(name="twitter:description"\s*\n?\s*content="[^"]*"\s*\n?\s*\/>)/;
  ...
  if (changed) {
    ...
    skippedCount++;
    console.log(`⏭️  ${path.relative(symbolsDir, filePath)} (already has tags)`);
  }
}
