...
for (const file of files) {
  ...
  const oldVerwandt = 'class="flex flex-wrap gap-3 justify-center mt-3 pt-3 pb-6"';
  const newVerwandt = 'class="flex flex-wrap gap-3 justify-center mt-4 pt-4 pb-0"';
  ...
  if (html.includes(oldGap)) {
    ...
    if (variantIdx === -1) {
      ...
      const altIdx = html.indexOf('Common Variants') !== -1 ? html.indexOf('Common Variants') :
                     html.indexOf('Variantes courantes') !== -1 ? html.indexOf('Variantes courantes') :
                     html.indexOf('Variantes comunes') !== -1 ? html.indexOf('Variantes comunes') :
                     html.indexOf('Частые варианты') !== -1 ? html.indexOf('Частые варианты') : -1;
      if (altIdx !== -1) {
        const gapIdx = html.indexOf(oldGap, altIdx);
        if (gapIdx !== -1 && gapIdx - altIdx < 500) {
          html = html.substring(0, gapIdx) + newGap + html.substring(gapIdx + oldGap.length);
          changed = true;
        }
      }
    } else {
